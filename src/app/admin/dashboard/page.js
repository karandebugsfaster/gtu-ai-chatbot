...
//       let data;
//       try { data = JSON.parse(text); } catch { toast.error('Failed to parse response'); return; }
...
//       const text = await res.text();
//       let data;
//       try { data = JSON.parse(text); } catch { toast.error('Server error during processing'); return; }
...
//         ));
//       }
//     } catch { toast.error('Network error during processing'); }
...
//       if (data.success) { toast.success('Deleted'); setDocuments(p => p.filter(d => d._id !== id)); }
...
//     if (file.size > 100 * 1024 * 1024)   { toast.error('File must be under 100MB');    e.target.value = ''; return; }
//     set('file', file);
//   };
...
//   //     if (data.success) {
//   //       toast.success('✅ Uploaded! Go to Manage tab → click ⚡ Process to enable AI.');
...
//   if (!selectedFile || !form.title || !form.branch || !form.semester || !form.subject) {
...
//   setUploading(true);
//   try {
//     // ✅ No slashes in publicId — Cloudinary treats / as folder separator
//     const publicId = `gtu_${form.branch}_sem${form.semester}_${Date.now()}`;
...
//     cloudinaryForm.append('access_mode', 'public'); // ✅ ADD THIS
//     // ✅ NO resource_type in form — it's in the URL only
...
//     setForm(p => ({ ...p, title: '', subject: '', author: '', file: null }));
...
//     borderRadius:'0.75rem', fontSize:'0.9375rem', color:'#111827',
...
//           <h1 style={{ fontSize:'1.875rem', fontWeight:'800', color:'#111827', marginBottom:'0.25rem' }}>Admin Dashboard</h1>
//           <p style={{ color:'#6b7280' }}>Signed in as <strong>{session?.user?.email}</strong></p>
//         </div>

//         {/* Tabs */}
//         <div style={{ display:'flex', gap:'0.5rem', marginBottom:'1.5rem', background:'white', padding:'0.375rem', borderRadius:'0.875rem', border:'1px solid #e5e7eb', boxShadow:'0 1px 3px rgba(0,0,0,0.05)' }}>
//           {tabs.map(t => (
//             <button key={t.id} onClick={() => setActiveTab(t.id)} style={{
//               flex:1, padding:'0.625rem 0.75rem',
//               background: activeTab === t.id ? 'linear-gradient(135deg,#6366f1,#9333ea)' : 'transparent',
//               color: activeTab === t.id ? 'white' : '#6b7280',
//               border:'none', borderRadius:'0.625rem', fontSize:'0.875rem', fontWeight:'600', cursor:'pointer',
//               display:'flex', alignItems:'center', justifyContent:'center', gap:'0.4rem', transition:'all 0.2s ease',
//             }}>
...
//             <h2 style={{ fontSize:'1.25rem', fontWeight:'700', color:'#111827', marginBottom:'0.5rem' }}>Upload Study Material</h2>
//             <div style={{ background:'#fffbeb', border:'1px solid #fde68a', borderRadius:'0.75rem', padding:'0.875rem 1rem', marginBottom:'1.75rem', fontSize:'0.875rem', color:'#92400e' }}>
//               💡 After uploading, go to <strong>Manage</strong> tab and click <strong>⚡ Process for AI</strong> to enable AI search on the document.
//             </div>

//             <form onSubmit={(e) => { e.preventDefault(); handleUpload(); }} style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
//               <div>
...
//                       padding:'0.5rem 1.25rem', border:`2px solid ${form.type===t?'#6366f1':'#e5e7eb'}`,
//                       borderRadius:'2rem', background:form.type===t?'#f5f3ff':'white',
//                       color:form.type===t?'#6366f1':'#374151', fontSize:'0.875rem', fontWeight:'600',
...
//                   padding:'2rem', border:`2px dashed ${form.file?'#6366f1':'#d1d5db'}`,
//                   borderRadius:'1rem', background:form.file?'#f5f3ff':'#fafafa', cursor:'pointer',
//                   transition:'all 0.2s ease', gap:'0.75rem',
//                 }}>
//                   {form.file ? (
//                     <>
...
//                 color: uploading ? '#9ca3af' : 'white', border:'none', borderRadius:'0.875rem',
//                 fontSize:'1rem', fontWeight:'700', cursor:uploading?'not-allowed':'pointer',
//                 transition:'all 0.2s ease', display:'flex', alignItems:'center', justifyContent:'center', gap:'0.5rem', marginTop:'0.5rem',
//               }}>
...
//               {[
//                 { label:'Type', key:'type', options:[['','All Types'],['pyq','PYQ'],['notes','Notes'],['book','Book'],['reference','Reference']] },
//                 { label:'Branch', key:'branch', options:[['','All Branches'],...BRANCHES.map(b=>[b.id,b.name])] },
//                 { label:'Semester', key:'semester', options:[['','All'],...[1,2,3,4,5,6,7,8].map(s=>[s,`Sem ${s}`])] },
//               ].map(({ label, key, options }) => (
//                 <div key={key} style={{ flex:1, minWidth:'130px' }}>
//                   <label style={lbl}>{label}</label>
//                   <select value={docFilter[key]} onChange={e => setDocFilter(p => ({...p,[key]:e.target.value}))}
...
//                 padding:'0.625rem 1.25rem', background:'linear-gradient(135deg,#6366f1,#9333ea)',
//                 color:'white', border:'none', borderRadius:'0.75rem', fontSize:'0.875rem',
...
//                 <p style={{ fontWeight:'600', color:'#111827' }}>No documents found</p>
//               </div>
//             ) : (
//               <>
...
//                   const sc = STATUS_STYLES[doc.processingStatus] || STATUS_STYLES.pending;
//                   const branchName  = BRANCHES.find(b => b.id === doc.branch)?.name || doc.branch;
//                   const isProc      = processing === doc._id;
//                   const needsProc   = doc.processingStatus === 'pending' || doc.processingStatus === 'failed';
...
//                         <span style={{ padding:'0.3rem 0.75rem', background:tc.bg, color:tc.color, borderRadius:'0.625rem', fontSize:'0.75rem', fontWeight:'800', flexShrink:0 }}>
//                           {tc.label}
//                         </span>
//                         <span style={{ flex:1, fontWeight:'700', color:'#111827', fontSize:'0.9375rem', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
//                           {doc.title}
//                         </span>
//                         <span style={{ padding:'0.2rem 0.625rem', background:sc.bg, color:sc.color, borderRadius:'2rem', fontSize:'0.6875rem', fontWeight:'700', flexShrink:0 }}>
//                           {sc.label}
//                         </span>
//                       </div>

//                       {/* Meta */}
//                       <div style={{ fontSize:'0.8125rem', color:'#9ca3af', display:'flex', gap:'0.5rem', flexWrap:'wrap', marginBottom:'1rem' }}>
//                         <span>{doc.subject}</span><span>·</span>
//                         <span>{branchName}</span><span>·</span>
...
//                         {/* ✅ Process button */}
//                         {needsProc && (
//                           <button onClick={() => handleProcess(doc._id)} disabled={isProc} style={{
//                             padding:'0.5rem 1rem',
//                             background: isProc ? '#f3f4f6' : 'linear-gradient(135deg,#10b981,#059669)',
//                             color: isProc ? '#9ca3af' : 'white',
//                             border:'none', borderRadius:'0.625rem', fontSize:'0.8125rem', fontWeight:'700',
//                             cursor: isProc ? 'not-allowed' : 'pointer',
//                             boxShadow: isProc ? 'none' : '0 2px 8px rgba(16,185,129,0.3)',
//                             display:'flex', alignItems:'center', gap:'0.375rem',
//                           }}>
//                             {isProc ? (
//                               <><svg style={{ width:'0.875rem',height:'0.875rem',animation:'spin 1s linear infinite' }} fill="none" viewBox="0 0 24 24">
...
//                           <button onClick={() => handleProcess(doc._id)} disabled={isProc} style={{
//                             padding:'0.5rem 0.875rem', background:'#f0fdf4', color:'#10b981',
...
//                           border:'none', borderRadius:'0.625rem', fontSize:'0.8125rem', fontWeight:'600', cursor:'pointer',
//                         }}>
//                           View PDF
//                         </button>

//                         <button onClick={() => handleDelete(doc._id)} disabled={deleting === doc._id} style={{
//                           padding:'0.5rem 0.875rem',
//                           background: deleting === doc._id ? '#f3f4f6' : '#fef2f2',
//                           color: deleting === doc._id ? '#9ca3af' : '#ef4444',
//                           border:'none', borderRadius:'0.625rem', fontSize:'0.8125rem', fontWeight:'600',
//                           cursor: deleting === doc._id ? 'not-allowed' : 'pointer',
//                         }}>
...
//             {[
//               { label:'Total',          value: documents.length,                                             icon:'📄', color:'#6366f1' },
...
//               { label:'AI Ready',       value: documents.filter(d=>d.processingStatus==='completed').length, icon:'✅', color:'#10b981' },
//               { label:'Needs Process',  value: documents.filter(d=>d.processingStatus==='pending').length,   icon:'⏳', color:'#f59e0b' },
//             ].map((s, i) => (
//               <div key={i} style={{ background:'white', borderRadius:'1rem', padding:'1.5rem', textAlign:'center', border:'1px solid #e5e7eb' }}>
...
export default function AdminDashboard() {
  ...
  const handleFileChange = (e) => {
    ...
    if (!file) return;
    if (file.type !== 'application/pdf') { toast.error('Only PDF files are allowed'); e.target.value = ''; return; }
    if (file.size > 50 * 1024 * 1024)   { toast.error('File must be under 50MB');    e.target.value = ''; return; }
    set('file', file);
  };
