...
export async function GET(request, context) {
  try {
    ...
    const fileName =
      `QP_${qp.subject || "paper"}_Sem${qp.semester || ""}_${Date.now()}.pdf`.replace(
        /[^a-zA-Z0-9_.-]/g,
        "_",
      );
...
function generateQuestionPaperPDF(qp) {
  ...
  doc.setFontSize(10);
  doc.text(
    `Total Marks: ${safeText(qp.totalMarks || qp.config?.totalMarks || 70)}`,
    20,
    y,
  );
  doc.text(
    `Duration: ${safeText(qp.examDuration || qp.config?.duration || 180)} mins`,
    150,
    y,
  );
  ...
  if (sections.length > 0) {
    ...
    sections.forEach((section) => {
      ...
      (section.questions || []).forEach((question) => {
        ...
        if (question.alternatives?.length > 0) {
          ...
          y += 5;
          doc.setFont(undefined, "normal");
          const orLines = doc.splitTextToSize(
            safeText(question.alternatives[0]),
            160,
          );
    ...
    qp.questions.forEach((q, i) => {
      ...
      const questionText =
        typeof q === "string" ? q : safeText(q.text || q.question || q);
  ...
  } else if (qp.rawContent) {
    // ✅ AI-generated paper — render raw text content
    doc.setFontSize(10);
    doc.setFont(undefined, "normal");

    const lines = qp.rawContent.split("\n");
    lines.forEach((line) => {
      if (y > 270) {
        doc.addPage();
        y = 20;
      ...
      const isHeader =
        /^(Q\.\d|GUJARAT|Semester|Subject|Time|Instructions|Q\d[\.\)])/i.test(
          trimmed,
        );
      ...
      y += wrapped.length * 6 + (isHeader ? 3 : 1);
    });
  } else {
